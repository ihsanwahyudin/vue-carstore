  ...
  <div id="modalCreate" class="hidden fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0" >
      ...
      <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
        ...
        <form action="" method="POST" enctype="multipart/form-data" v-on:submit="storeData">
          <input type="hidden" name="status" value="store">
          <section class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <span class="w-full">
                <h2 class="text-2xl mb-3">Add new Car</h2>
              </span>
              <span class="w-full text-sm">
                <label class="block mb-1 mx-1" for="">Merk</label>
                <input v-model="formData.merk" name="merk" class="border w-full p-2 rounded-md focus:outline-none focus:ring" type="text" :class="{'border-red-500': errors.merk, 'border-blue-300': !errors.merk}">
                <small v-if="errors.merk" class="text-red-500">{{ errors.merk[0] }}</small>
              </span>
              <span class="w-full text-sm">
                <label class="block mb-1 mx-1" for="">Type</label>
                <input v-model="formData.type" name="type" class="border w-full p-2 rounded-md focus:outline-none focus:ring" type="text" :class="{'border-red-500': errors.type, 'border-blue-300': !errors.type}">
                <small v-if="errors.type" class="text-red-500">{{ errors.type[0] }}</small>
              </span>
              <span class="w-full text-sm">
                <label class="block mb-1 mx-1" for="">Warna</label>
                <input v-model="formData.warna" name="warna" class="w-10 h-10 mx-2 border rounded-md focus:outline-none focus:ring" type="color">
              </span>
              <span class="w-full text-sm">
                <label class="block mb-1 mx-1" for="">Harga</label>
                <input v-model="formData.harga" name="harga_mobil" class="format-number disable-letter border w-full p-2 rounded-md focus:outline-none focus:ring" type="text" :class="{'border-red-500': errors.harga_mobil, 'border-blue-300': !errors.harga_mobil}">
              ...
              <span>
                ...
                <div class="input-file border rounded-md overflow-hidden" :class="{'border-red-500': errors.gambar, 'border-blue-300': !errors.gambar}">
                  <input type="file" name="gambar" class="dropify rounded-md" data-allowed-file-extensions="jpg jpeg png" />
                </div>
          ...
          <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            ...
            <button v-on:click="modalCreate" type="button" class="btn-toggle-create mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
              Cancel
            </button>
          </div>
        </form>
